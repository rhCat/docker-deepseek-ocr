...
ENV USE_CUDA=1 USE_NCCL=1 USE_MKLDNN=0 USE_DISTRIBUTED=1 USE_CUDNN=0 \
    TORCH_CUDA_ARCH_LIST="8.0;8.6;8.9;9.0;9.0+PTX;9.0a+PTX" \
    MAX_JOBS=8 \
...
RUN apt-get update && apt-get install -y --no-install-recommends libopenblas0 && \
    rm -rf /var/lib/apt/lists/*
RUN TORCH_CUDA_ARCH_LIST="8.0;8.6;8.9;9.0;9.0+PTX;9.0a+PTX" python3 -m pip install /tmp/torch-2.6.0*.whl

# now build torchvision
RUN TORCH_CUDA_ARCH_LIST="8.0;8.6;8.9;9.0;9.0+PTX;9.0a+PTX" python3 setup.py bdist_wheel
